...
def coco_h36m_simple(keypoints):
    ...
    keypoints_h36m[7, 0] += 2 * (
            keypoints_h36m[7, 0] - np.mean(keypoints_h36m[[0, 8], 0], axis=0, dtype=np.float32))
    keypoints_h36m[8, 1] -= (np.mean(keypoints[1:3, 1], axis=0, dtype=np.float32) - keypoints[0, 1]) * 2 / 3

    return keypoints_h36m

def h36m_coco_format_simple(keypoints, scores):
    assert len(keypoints.shape) == 2 and len(scores.shape) == 2
    new_score = np.zeros_like(scores, dtype=np.float32)
    kpts = coco_h36m_simple(keypoints)
    new_score[h36m_coco_order] = scores[coco_order]
    new_score[0] = np.mean(scores[[11, 12]], axis=0, dtype=np.float32)
    new_score[8] = np.mean(scores[[5, 6]], axis=0, dtype=np.float32)
    new_score[7] = np.mean(new_score[[0, 8]], axis=0, dtype=np.float32)
    new_score[10] = np.mean(scores[[1, 2, 3, 4]], axis=0, dtype=np.float32)
    return kpts, new_score
